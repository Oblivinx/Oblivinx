...
export class Collection<TSchema extends Document = Document> {
  ...
  async insertMany(
    ...
    for (let i = 0; i < total; i += chunkSize) {
      ...
      try {
        const idsJson = wrapNative(() =>
          native.insertMany(this.#db._handle, this.#name, JSON.stringify(slice)),
        );
        const ids: string[] = JSON.parse(idsJson) as string[];
    ...
    if (options.durability === 'fast'
      && typeof (this.#db as unknown as { checkpoint?: () => Promise<void> }).checkpoint === 'function') {
      await (this.#db as unknown as { checkpoint: () => Promise<void> }).checkpoint();
    }
