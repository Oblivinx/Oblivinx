 ...
 * ```
 */
export class UpdateBuilder<T extends Document = Document> {
  /** Accumulated update operators @internal */
  readonly #ops: Record<string, Record<string, unknown>>;

  /**
   * Private constructor — use `UpdateBuilder.create()`.
   * @internal
   */
  private constructor(ops: Record<string, Record<string, unknown>> = {}) {
    this.#ops = ops;
  }
  ...
  setMany(fields: Partial<Record<keyof T | string, unknown>>): UpdateBuilder<T> {
    let builder: UpdateBuilder<T> = this;
   ...
   * @returns New UpdateBuilder
   */
  pop(field: keyof T | string, direction: -1 | 1 = 1): UpdateBuilder<T> {
   ...
   * @internal
   */
  #addOp(operator: string, field: string, value: unknown): UpdateBuilder<T> {
    const existingOp = this.#ops[operator] ?? {};
    const newOps = {
      ...this.#ops,
      [operator]: { ...existingOp, [field]: value },
    };
    return new UpdateBuilder<T>(newOps);
  }
}
