...
export class BackupManager {
   ...
   * ```
   */
  async createFull(destPath: string): Promise<BackupInfo> {
    ...
    const exportData = JSON.parse(exportJson) as Record<string, unknown[]>;
    const collectionNames = Object.keys(exportData);
    const documentCount = collectionNames.reduce(
      (sum, name) => sum + (exportData[name]?.length ?? 0),
      0,
    );
   ...
   * @returns BackupInfo
   */
  async createIncremental(destPath: string): Promise<BackupInfo> {
   ...
   * ```
   */
  async exportLogical(destPath: string): Promise<BackupInfo> {
    const startTime = Date.now();
  ...
  async importLogical(
    ...
    for (const colName of collections) {
      ...
      if (options.dropExisting) {
        try { await col.drop(); } catch { /* collection may not exist */ }
      }
   ...
   * ```
   */
  async verify(backupPath: string): Promise<{
    valid: boolean;
    sizeBytes: number;
    issues: string[];
  }> {
   ...
   */
  async list(backupDir: string): Promise<Array<{
    path: string;
    type: 'full' | 'logical';
    sizeBytes: number;
    createdAt: Date;
    ...
    for (const entry of entries) {
      ...
      if (ext !== '.ovnbak' && ext !== '.json') continue;
   ...
   * ```
   */
  async prune(backupDir: string, policy: PrunePolicy): Promise<number> {
    ...
    if (policy.maxCount !== undefined && backups.length > policy.maxCount) {
      ...
      for (const b of excess) toDelete.push(b.path);
    }
    ...
    if (policy.maxAgeDays !== undefined) {
      ...
      for (const b of backups) {
        if (now - b.createdAt.getTime() > maxAgeMs) {
          if (!toDelete.includes(b.path)) toDelete.push(b.path);
        }
      }
    }
