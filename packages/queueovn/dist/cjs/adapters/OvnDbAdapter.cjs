...
function fromDoc(doc) {
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const { _id, id: _ignored, ...rest } = doc;
    return { id: _id, ...rest };
}
/**
...
class OvnDbAdapter {
    ...
    async getRecoveryCheckpoint() {
        this.checkInit();
        if (typeof this.db.getRecoveryCheckpoint !== 'function')
            return null;
    ...
    async setRecoveryCheckpoint(cp) {
        this.checkInit();
        if (typeof this.db.setRecoveryCheckpoint !== 'function')
            return;
    ...
    getDLQStore() {
        ...
        return {
            add: async (jobId, entry) => {
                ...
                if (modifiedCount === 0) {
                    try {
                        ...
                        await c.updateOne({ _id: jobId }, { $set: doc });
                    }
                }
            },
            remove: async (jobId) => {
            ...
            list: async () => {
                const docs = (await col().find({}, { sort: { failedAt: -1 } }));
                return docs.map((d) => ({
                    job: d.job,
    ...
    async initialize() {
        let OvnClass;
        try {
            const mod = await Promise.resolve(`${'oblivinx3x'}`).then(s => __importStar(require(s)));
            ...
            throw new AdapterError_js_1.AdapterError('OvnDbAdapter requires "oblivinx3x" to be installed. Run: npm install oblivinx3x', err);
        }
        ...
        if (this.options.compression)
            dbOptions.compression = this.options.compression;
        if (this.options.bufferPool)
            dbOptions.bufferPool = this.options.bufferPool;
        if (this.options.durability)
            dbOptions.durability = this.options.durability;
     ...
     */
    async ensureIndexes() {
        const tryCreate = async (col, fields, options = {}) => {
            try {
                await col.createIndex(fields, { background: true, ...options });
            }
            catch (err) {
                const msg = String(err?.message || '').toLowerCase();
                if (msg.includes('already exists') || err?.code === 85 || err?.code === 86)
                    return;
                throw err;
            }
        };
        // 1. Composite index: primary query path for pop() and size()
        await tryCreate(this.collection, { state: 1, runAt: 1, priority: 1, createdAt: 1 });
        // 2. Partial index: only covers PENDING+RETRYING docs → faster pop() when queue is busy
        await tryCreate(this.collection, { runAt: 1, priority: 1, createdAt: 1 }, { partialFilterExpression: { state: { $in: ['pending', 'retrying'] } } });
        // 3. TTL index: Oblivinx3x auto-deletes docs whose expiresAt has passed.
        await tryCreate(this.collection, { expiresAt: 1 }, { expireAfterSeconds: 0, sparse: true });
    ...
    async push(job) {
        ...
        const { modifiedCount } = await this.collection.updateOne({ _id: id }, { $set: doc });
        if (modifiedCount === 0) {
    ...
    async pop() {
        ...
        for (const doc of docs) {
            ...
            const result = await this.collection.updateOne({ _id: doc._id, state: { $in: ['pending', 'retrying'] } }, { $set: { state: 'active', startedAt: now } });
            if (result.modifiedCount > 0) {
                return { ...fromDoc(doc), state: 'active', startedAt: now };
            }
        }
        return null;
    }
    ...
    async stats() {
        ...
        return Object.fromEntries(results.map((r) => [r._id ?? 'unknown', r.count]));
    }
    checkInit() {
        if (!this.db || !this.collection) {
            throw new AdapterError_js_1.AdapterError('OvnDbAdapter not initialized. Call initialize() first.');
        }
    }
}
exports.OvnDbAdapter = OvnDbAdapter;
