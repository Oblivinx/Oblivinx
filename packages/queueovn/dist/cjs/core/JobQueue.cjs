...
class JobQueue {
    ...
    constructor(config) {
        ...
        this.flowController = new FlowController_js_1.FlowController(this.cfg.adapter, this.wal, () => this.startProcessLoop());
    ...
    async enqueueBatch(jobs, options) {
        ...
        const now = clock_js_1.systemClock.now();
        for (const opts of jobs) {
            const job = (0, Job_js_1.createJob)(opts, {
                defaultPriority: this.cfg.defaultPriority,
                defaultMaxAttempts: this.cfg.defaultMaxAttempts,
                defaultMaxDuration: this.cfg.defaultMaxDuration,
            });
        ...
        for (const j of built)
            this.emitter.emit(QueueEvents_js_1.QueueEvent.ENQUEUED, j);
        return ids;
    }
    ...
    // Keep imports alive
    static _imports = { AdapterError: AdapterError_js_1.AdapterError, QueueError: QueueError_js_1.QueueError, JobTimeoutError: JobTimeoutError_js_1.JobTimeoutError, DiscardJobError: DiscardJobError_js_1.DiscardJobError };
}
