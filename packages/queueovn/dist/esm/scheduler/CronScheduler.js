...
export class CronScheduler extends EventEmitter {
    ...
    async initialize() {
        if (this.db) {
            ...
            await this.schedCol.createIndex({ id: 1 }, { unique: true, background: true }).catch(() => { });
    ...
    _persistEntryToDb(entry) {
        if (!this.schedCol)
            return;
        this.schedCol.updateOne({ _id: entry.id }, { $set: { ...entry, _id: entry.id } }).catch(() => { });
    }
    /** Delete a schedule entry from the Oblivinx3x collection (fire-and-forget). */
    _deleteEntryFromDb(id) {
        if (!this.schedCol)
            return;
        this.schedCol.deleteOne({ _id: id }).catch(() => { });
    }
    ...
    async _loadFromDb() {
        if (!this.schedCol)
            return;
        try {
            const docs = await this.schedCol.find({});
