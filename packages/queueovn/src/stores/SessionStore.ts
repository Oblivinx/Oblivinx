...
export class SessionStore {
    ...
    get<T = Record<string, unknown>>(userJid: string, groupJid: string): SessionData<T> | null {
        return this.kv.get<SessionData<T>>(this._key(userJid, groupJid))
    }
     ...
     * ```
     */
    advance<T = Record<string, unknown>>(
        userJid: string,
        groupJid: string,
        nextStep: string,
        dataPatch?: Partial<T>,
        ...
        const session = this.get<T>(userJid, groupJid)
    ...
    patch<T = Record<string, unknown>>(
        ...
        const session = this.get<T>(userJid, groupJid)
        if (!session) return null
        return this.advance<T>(userJid, groupJid, session.step, dataPatch, ttlMs)
    }
     ...
     */
        ...
        if (existed) this.pubsub?.publish('session:end', { userJid, groupJid })
        return existed
    }
    ...
    forGroup<T = Record<string, unknown>>(groupJid: string): SessionData<T>[] {
        ...
        return keys.map(k => this.kv.get<SessionData<T>>(k)).filter(Boolean) as SessionData<T>[]
    }
    ...
    endAll(): number {
        ...
        if (count > 0) this.pubsub?.publish('session:end-all', { count })
        return count
    }
     ...
     * ```
     */
    async ifStep<T = Record<string, unknown>>(
        userJid: string,
        groupJid: string,
        expectedStep: string,
        nextStep: string | null,
        fn: (session: SessionData<T>) => Promise<Partial<T> | void> | Partial<T> | void,
    ): Promise<{ matched: boolean; session: SessionData<T> | null }> {
        const session = this.get<T>(userJid, groupJid)
        if (!session || session.step !== expectedStep) return { matched: false, session: null }
        ...
        const updated = this.advance<T>(userJid, groupJid, nextStep, patch as Partial<T>)
        return { matched: true, session: updated }
    }

    // ─── Private helpers ───────────────────────────────────────────────────────

    private _key(userJid: string, groupJid: string): string {
