...
export class SortedSet extends EventEmitter {
    ...
    zrem(key: string, ...members: string[]): number {
        ...
        if (count > 0) this.sortedCache.delete(key)
        return count
    }
    ...
    zrange(key: string, start: number, stop: number, withScores?: boolean): string[] | ZMember[] {
        const sorted = this._sorted(key)
        const len = sorted.length
        const s = start < 0 ? Math.max(0, len + start) : start
        const e = stop < 0 ? len + stop : Math.min(stop, len - 1)
        const slice = sorted.slice(s, e + 1)
        if (withScores === true) return slice
    ...
    zrevrange(key: string, start: number, stop: number, withScores?: boolean): string[] | ZMember[] {
     ...
     * ```
     */
    globalLeaderboard(gameType: string, limit = 10): Array<{ rank: number; member: string; score: number; groups: number }> {
        const prefix = `lb:${gameType}:`
        const aggregated = new Map<string, { score: number; groups: number }>()
    ...
    private _getOrCreate(key: string): Map<string, number> {
        let set = this.sets.get(key)
        if (!set) { set = new Map(); this.sets.set(key, set) }
        return set
    }

    private _sorted(key: string): ZMember[] {
        ...
        const sorted: ZMember[] = []
        for (const [member, score] of set) sorted.push({ member, score })
    ...
    private _save(): void {
        ...
        try {
            ...
            writeFileSync(this.persistPath, JSON.stringify({ v: 1, data }), 'utf8')
        } catch { /* non-fatal */ }
    }
    ...
    private _load(): void {
        ...
        try {
            ...
            for (const [key, members] of Object.entries<Record<string, number>>(raw.data ?? {})) {
                const set = new Map<string, number>()
                for (const [m, s] of Object.entries(members)) set.set(m, s)
