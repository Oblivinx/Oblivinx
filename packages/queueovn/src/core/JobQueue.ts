...
export class JobQueue {
    ...
    private readonly recovery: Recovery | null;
    private readonly nativeWAL: boolean;
    ...
    constructor(config: QueueConfig) {
        ...
        const adapterMark =
            (this.cfg.adapter as { usesNativeWAL?: boolean }).usesNativeWAL === true;
        ...
        if (dlqPlugin) {
            ...
            const adapterAny = this.cfg.adapter as { getDLQStore?: () => unknown };
            if (typeof adapterAny.getDLQStore === 'function') {
                dlqPlugin.setStore(adapterAny.getDLQStore() as Parameters<DeadLetterQueue['setStore']>[0]);
            }
        }
    ...
    async initialize(): Promise<void> {
        ...
        if (typeof (this.cfg.adapter as any).initialize === 'function') {
            await (this.cfg.adapter as any).initialize();
        }
        ...
        if (typeof (this.cfg.adapter as any).setOnNewJob === 'function') {
            (this.cfg.adapter as any).setOnNewJob(() => this.startProcessLoop());
        }
    ...
    async enqueueBatch<T extends JobPayload>(
        ...
        const ids: string[] = [];
        const built: Job<JobPayload>[] = [];
        const delayed: Job<JobPayload>[] = [];
        const ready: Job<JobPayload>[] = [];
        ...
        for (let i = 0; i < ready.length; i += chunkSize) {
            ...
            if (typeof adapter.pushMany === 'function') {
                ...
                for (const j of slice) await adapter.push(j);
            }
            if (this.cfg.persistence.enabled && this.wal) {
                for (const j of slice) this.wal.append('ENQUEUE', j.id, j);
            }
        ...
        this.startProcessLoop();
        for (const j of built) this.emitter.emit(QueueEvent.ENQUEUED, j);
        return ids;
    }
    ...
    async drain(): Promise<void> {
        ...
        if (size === 0 && this.running === 0) return;
     ...
     */
    async runInProcess<T extends JobPayload, R = unknown>(
        type: string,
        payload: T,
        options?: Omit<JobOptions<T>, 'type' | 'payload'>,
    ): Promise<R> {
        this.checkOpen();
        ...
        try {
            ...
            handlerResult = await Promise.race([
                handler(job.payload, ctx) as Promise<R>,
                timeoutPromise,
    ...
    private startProcessLoop(): void {
        if (this.processingLoop || this.isClosed || this.isPaused) return;
    ...
    private async runProcessLoop(): Promise<void> {
        try {
            while (!this.isClosed && !this.isPaused && this.running < this.cfg.workers.max) {
                ...
                if (!job) break; // Queue empty — exit loop, wait for next trigger
     ...
     */
    private dispatchJob(job: Job<JobPayload>): void {
        const wrapper = async (): Promise<void> => {
            try {
                ...
                this.emitter.emit(
                    QueueEvent.ERROR,
                    err instanceof Error ? err : new QueueError(String(err)),
                );
            } finally {
    ...
    private async onFailure<T extends JobPayload>(job: Job<T>, error: Error): Promise<void> {
        ...
        const retryPolicy: IRetryPolicy = job.retryPolicy ?? this.defaultRetry;
    ...
    private checkDrain(): void {
        ...
        if (this.running > 0 || this.processingLoop || this.scheduler.size > 0) return;
    ...
    // Keep imports alive
    private static _imports = { AdapterError, QueueError, JobTimeoutError, DiscardJobError };
}
