...
impl OvnEngine {
    ...
    pub fn define_relation(&self, definition: &serde_json::Value) -> OvnResult<()> {
        ...
        let rel: RelationDefinition = serde_json::from_value(definition.clone())
            .map_err(|e| OvnError::ValidationError(format!("Invalid relation definition: {}", e)))?;
        ...
        log::info!(
            ...
            rel.from_collection, rel.from_field,
            rel.to_collection, rel.to_field,
            rel.relation_type, rel.on_delete
        );
        Ok(())
    }
    ...
    pub fn drop_relation(&self, from: &str, to: &str) -> OvnResult<()> {
        ...
        if relations.len() == before {
            return Err(OvnError::ValidationError(
                format!("Relation from '{}' to '{}' not found", from, to)
            ));
        }
    ...
    pub fn set_referential_integrity(&self, mode: &str) -> OvnResult<()> {
        ...
        match mode {
            ...
            _ => Err(OvnError::ValidationError(
                format!("Invalid integrity mode: '{}'. Must be 'off', 'soft', or 'strict'", mode)
            ))
        }
    }
    ...
    pub fn validate_referential_integrity(&self, doc: &ObeDocument, collection: &str) -> OvnResult<()> {
        ...
        for rel in relations.iter() {
            ...
            if rel.from_collection == collection {
                if let Some(ref_value) = doc.get_path(&rel.from_field) {
                    ...
                    let found = self.lookup_document_by_id(
                        &rel.to_collection,
                        &rel.to_field,
                        &ref_id
                    )?;
                    ...
                    if !found && mode == "strict" {
                        return Err(OvnError::ValidationError(format!(
                            ...
                            collection, rel.from_field, ref_id,
                            rel.to_collection, rel.to_field
                        )));
    ...
    pub fn handle_cascade_delete(&self, collection: &str, doc_id: &serde_json::Value) -> OvnResult<()> {
        let relations = self.relations.lock().unwrap();
        let cascade_targets: Vec<_> = relations.iter()
            .filter(|r| r.to_collection == collection 
                && matches!(r.on_delete, OnDeleteAction::Cascade))
            .cloned()
            .collect();
        drop(relations);
