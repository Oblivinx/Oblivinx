...
impl MvccManager {
    ...
    pub fn commit(&self, txid: u64) -> OvnResult<()> {
        ...
        let txn = active.get(&txid).ok_or_else(|| OvnError::TransactionAborted {
            txid,
    ...
    pub fn add_version(&self, key: Vec<u8>, entry: VersionEntry) {
        ...
        let chain = chains.entry(key).or_insert_with(Vec::new);
    ...
    fn update_horizon(&self) {
        ...
        let min_txid = active.keys().copied().min().unwrap_or(self.txid_gen.current());
...
mod tests {
    ...
    fn test_mvcc_transaction_isolation() {
        ...
        mvcc.add_version(
            b"doc1".to_vec(),
            VersionEntry { txid: txid1, data: b"v1".to_vec(), tombstone: false },
        );
        ...
        mvcc.add_version(
            b"doc1".to_vec(),
            VersionEntry { txid: txid2, data: b"v2".to_vec(), tombstone: false },
        );
